...
namespace BETMart.BLL.Notifications
{
    ...
    public class RazorMailerEngine : IDisposable
    {
        ...
        /// <param name="mailService">The method by which to send the email.  Custom dispatchers can be implemented using the IEmailDispatcher interface</param>
        public RazorMailerEngine(string templatePath, string fromEmail, string fromName, IMailService mailService)
        {
            _fromName = fromName;
        ...
        public virtual async Task SendAsync(MailMessage message)
        {
            try
            {
                if (_mailService == null)
                    throw new Exception("This RazorMailerEngine instance was constructed without a IEmailDispatcher and thus can't send MailMessages");
        ...
        public virtual MailMessage CreateMailMessage(string to, string subject, string body, Attachment[] attachments = null)
        {
            try
            {
                if (string.IsNullOrEmpty(_fromEmail))
                    throw new Exception("This RazorMailerEngine instance was constructed without a 'From Email' and thus can't send MailMessages");

                if (string.IsNullOrEmpty(_fromEmail))
                    throw new Exception("This RazorMailerEngine instance was constructed without a 'From Name' and thus can't send MailMessages");
